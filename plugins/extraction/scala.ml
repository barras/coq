...
(* | None -> str (name_of_tvar2 i)*)
    ...
    | Tglob (r, l) ->
 if is_infix r && List.length l == 2 then
 let s = T.find_custom r in
...
let rec pp_expr (tvs: Id.t list) (env: C.env)  : ml_ast -> 'a =
  function
    | MLrel (i, ts) ->
       ...
       in
       pr_id id ++ type_annot
    |MLapp ((MLglob (r, ty_args)), [arg1; arg2]) when is_infix r->
      let s = T.find_custom r in
      hov 2 (str "(" ++ pp_expr tvs env (arg1) ++ spc()
             ++ str (String.sub s 1 (String.length s - 2)) ++    str " "
             ++ pp_expr tvs env (arg2) ++ str ")")
    ...
    | MLcons (_, r, args) ->
       if is_infix r && List.length args == 2 then
         let s = T.find_custom r in
         hov 2 (str "(" ++ pp_expr tvs env (List.hd args) ++ spc()
                ++ str (String.sub s 1 (String.length s - 2))
                ++ str " " ++ pp_expr tvs env (List.nth args 1) ++ str ")")
  ...
  | Dind (kn, ind) ->
     ...
     let rec iter i =
       if i >= Array.length ind.ind_packets then mt()
         ...
         let ip = (mind,i) in
         if T.is_custom (GlobRef.IndRef ip) then iter (i+1)
         else
